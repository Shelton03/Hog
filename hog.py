...
partner_one_email = "sheltonmutambirwa@gmail.com"
partner_two_email = ""
assignment = "hog"
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score<100 and opponent_score<100:
        if other(who):
            ...
            points=take_turn(num_rolls,score,dice)
            opponent_score=points+opponent_score
            if score*2==opponent_score or opponent_score*2==score:
                p=score
                k=opponent_score
                opponent_score=p
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    tracker=0
    while num_rolls<=10:
        score =make_averaged(roll_dice)(num_rolls,dice)
        if score>tracker:
            rolls=num_rolls
        print(num_rolls, 'dice scores', round(score,1) ,'on average')
...
def swap_strategy(score, opponent_score):
    ...
    5
    """
    if (score+max(opponent_score%10,opponent_score//10)+1)*2==opponent_score:
        num_rolls=0
    elif(score+max(opponent_score%10,opponent_score//10)+1)/opponent_score==2.0:
        num_rolls=BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
   ...
   ***When playing from behind,the player looks to get as many beneficial swaps as possible.
   If a beneficial swap isn't possible, the player uses the number of rolls which average the highest points.
   The last part of the strategy when behind is to try and make the other player roll a four_sided die, when in
   this becomes the first goal.Otherwise to be safe, the player either implements the bacon strategy or rolls 7 times.
    """
    max_yield_roll=max_scoring_num_rolls(select_dice(score,opponent_score))
    roll_points= score+(max(opponent_score//10,opponent_score%10)+1)
    a=max_yield_roll
    if score<opponent_score:
        if swap_strategy(score,opponent_score)==0:
            ...
            a=max_yield_roll
        if (roll_points+opponent_score)%7==0:
            a=0
    if score>opponent_score:
        if (roll_points+opponent_score)%7==0:
            a=0
